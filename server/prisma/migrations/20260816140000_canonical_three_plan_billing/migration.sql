...
DO $constraints$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'BillingPayment_plan_or_credit_pack') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'BillingWebhookEvent_payload_bounded') THEN
    ALTER TABLE "BillingWebhookEvent" ADD CONSTRAINT "BillingWebhookEvent_payload_bounded" CHECK (octet_length("payload"::text) <= 32768);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'BillingPayment_instituteId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'PlanTrialClaim_instituteId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'BillingWebhookEvent_instituteId_fkey') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'PlanNotification_instituteId_fkey') THEN
