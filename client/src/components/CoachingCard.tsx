  ...
  return (
    <div className="group bg-white rounded-2xl border border-neutral-200/80 shadow-sm hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 flex flex-col overflow-hidden relative">
      ...
      <div className="p-5 pb-0 flex items-start gap-3.5">
        ...
        <div className="flex-1 min-w-0 pt-0.5">
          ...
          <p className="text-xs text-neutral-500 font-medium flex items-center gap-1.5 mt-0.5">
            ...
            {coaching.isVerified ? (
              <span className="inline-flex items-center gap-0.5 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-200">
                <CheckCircle2 className="w-3 h-3 text-blue-500 shrink-0" />
                Verified
              </span>
            ) : (
              <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-neutral-500 bg-neutral-100 px-2 py-0.5 rounded-full border border-neutral-200" title="Public directory listing. Claim profile available.">
                Directory
              </span>
            )}
          </p>
      ...
      <div className="px-5 mt-3 flex flex-wrap items-center gap-1.5">
        {/* Classes Badge */}
        {coaching.classesOffered && coaching.classesOffered.length > 0 && (
          <span className="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-amber-50 text-amber-800 border border-amber-200/80">
            🎓 {coaching.classesOffered.slice(0, 3).join(', ')}{coaching.classesOffered.length > 3 ? ` +${coaching.classesOffered.length - 3}` : ''}
          </span>
        )}
        ...
        {coaching.subjectsOffered && coaching.subjectsOffered.length > 0 ? (
          <>
            {coaching.subjectsOffered.slice(0, 2).map((sub, i) => (
              <span
                key={i}
            ...
            {coaching.subjectsOffered.length > 2 && (
              <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-neutral-100 text-neutral-500 border border-neutral-200/80">
                +{coaching.subjectsOffered.length - 2} more
              </span>
            )}
          </>
        ) : (
