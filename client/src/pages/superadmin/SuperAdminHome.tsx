...
import { AlertTriangle, ArrowRight, CheckCircle2, Clock3, Headphones, RefreshCw, ShieldAlert, Star, UsersRound } from 'lucide-react';
...
import { buildHomeMetrics, visibleHomeAttention } from '../../features/superadmin-shell/supportVisibility';
...
export default function SuperAdminHome() {
  ...
  const attention = useMemo(() => visibleHomeAttention(data?.attention || []), [data]);
  const grouped = useMemo(() => ({
    critical: attention.filter(item => item.severity === 'CRITICAL'),
    today: attention.filter(item => item.severity === 'TODAY'),
    upcoming: attention.filter(item => item.severity === 'UPCOMING')
  }), [attention]);
  ...
  if (!data) return null;
  const metrics = buildHomeMetrics(data);
