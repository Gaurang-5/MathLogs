...
export default function ScanScreen() {
  const { testId, maxMarks } = useLocalSearchParams();
  ...
  const handleBarCodeScanned = async ({ data }: { data: string }) => {
    ...
    try {
      const res = await api.get(`/students/lookup/${encodeURIComponent(data)}?testId=${testId}`);
      const st = res.data;
      const existing = st.marks?.find((m: any) => m.testId === testId);
      ...
      <SafeAreaView edges={['top']} style={s.overlay}>
        <View style={s.header}>
          ...
          <View style={s.titleWrap}>
            <Text style={s.title}>Scan OMR QR</Text>
          </View>
        </View>
        ...
        <View style={s.targetArea}>
           ...
           <Text style={s.targetText}>Align full sticker (QR + Marks) in frame</Text>
        </View>
      </SafeAreaView>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
            <View style={s.modalContent}>
              ...
              <View style={s.inputWrap}>
                 ...
                 />
                 <Text style={s.maxMarksText}>/ {maxMarks}</Text>
              </View>

              <TouchableOpacity 
                style={[s.saveBtn, processing && { opacity: 0.5 }]} 
                onPress={handleSave}
